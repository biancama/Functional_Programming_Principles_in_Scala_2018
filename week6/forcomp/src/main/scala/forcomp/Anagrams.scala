...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.toLowerCase.groupBy(c => c).map{case(c, s) => (c, s.size)}.toList sortBy{case (c, i) => c}
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def iter(occurrences: Occurrences): List[Sentence] = if (occurrences.isEmpty) List(Nil) else
      for {
        combination <- combinations(occurrences)
        word <- dictionaryByOccurrences getOrElse(combination, Nil)
        otherSentences <- iter(subtract(occurrences, wordOccurrences(word)))
        //_ = println(word)
        if !combination.isEmpty
      } yield word :: otherSentences
